...
describe(`tree`, () => {
  ...
  it(`has the expected api`, () => {
    ...
    expect(typeof channel.prepare).toEqual(`function`)
  })

  it(`throws if listener is not an object`, () => {
    const channel = tree()
  ...
  it(`supports multiple subscriptions`, () => {
    ...
    let unsubscribeA = channel.subscribe({ a: listenerA })
    channel.broadcast()
    ...
    const unsubscribeB = channel.subscribe({ b: listenerB })
    expect(listenerA.mock.calls.length).toBe(2)
    expect(listenerB.mock.calls.length).toBe(0)
    ...
    unsubscribeA = channel.subscribe({ a: listenerA })
    expect(listenerA.mock.calls.length).toBe(3)
    expect(listenerB.mock.calls.length).toBe(2)
  ...
  it(`only removes listener once when unsubscribe is called`, () => {
    ...
    const unsubscribeA = channel.subscribe({ a: listenerA })
  ...
  it(`notifies all subscribers about current broadcast regardless if any of them gets unsubscribed in the process`, () => {
    ...
    unsubscribeHandles.push(channel.subscribe({ a: () => listener1() }))
    ...
    unsubscribeHandles.push(channel.subscribe({ a: () => listener3() }))
  ...
  it(`notifies only subscribers active at the moment of current broadcast`, () => {
    ...
    const maybeAddThirdListener = () => {
      if (!listener3Added) {
        listener3Added = true
        channel.subscribe({ a: () => listener3() })
      }
    }

    channel.subscribe({ a: () => listener1() })
    channel.subscribe({
      a: () => {
        listener2()
  ...
  it(`uses the last snapshot of subscribers during nested broadcast`, () => {
    ...
    const unsubscribe1 = channel.subscribe({
      a: () => {
        ...
        unsubscribe1()
        unsubscribe4 = channel.subscribe({ a: listener4 })
        channel.broadcast()
  ...
  test(`prepare`, () => {
    ...
    channel.subscribe({ a: listener, b: listener })

    const broadcast = channel.prepare(({ a }) => ({ a }))
